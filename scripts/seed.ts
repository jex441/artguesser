...
async function fetchWithRetry(
	...
	for (let attempt = 0; attempt <= retries; attempt++) {
		...
		if (res.status === 403 || res.status === 429) {
			...
			console.log(
				`  [Met] HTTP ${res.status} — waiting ${delay / 1000}s before retry ${attempt + 1}/${retries}...`,
			);
...
function paintingDepartmentId(nationality: string | undefined): number | null {
	...
	if (
		...
		n.includes("austrian") ||
		n.includes("belgian") ||
		n.includes("norwegian") ||
		n.includes("russian") ||
		n.includes("belarusian") ||
		n.includes("greek") ||
		n.includes("swiss") ||
		n.includes("mexican")
	)
...
function metArtistMatches(obj: any, artistName: string): boolean {
	...
	const normalized = (s: string) =>
		s
			...
			.trim();
	return (
		normalized(display).includes(normalized(artistName)) ||
...
async function seedPaintingsFromMet(
	...
	const objectIds = await fetchMetArtworksByArtist(
		artist.name,
		artist.nationality,
	);
	let count = 0;
